...
* Custom PCB design in Altium Designer (in progress)
...
| Input   | 2x USB-C    | One port used for programming the MCU, other used for use as a speaker         |
| Output   | 4x PCB Headers    | Left and right DAC output signals, plus two dedicated ground headers for oscilloscope probe referencing         |
...
**22Ω series resistors on USB D+/D−:** Standard signal integrity practice for USB. The series resistors damp impedance mismatches and reduce reflections on the differential data lines.
...
| MODE SWITCH | GPIO 9 | Mode selection switch |

### Schematics
All schematics were captured in Altium Designer. PCB layout is currently in progress.

<img width="800" alt="Sheet 1" src="https://github.com/user-attachments/assets/33573453-8741-4d82-a884-d7a442b58595" />
...
<img width="800" alt="Sheet 2" src="https://github.com/user-attachments/assets/e32977b2-24a0-424a-b109-ff45de509878" />
...
<img width="800" alt="Sheet 3" src="https://github.com/user-attachments/assets/55a307ce-984d-4cda-8e44-17ea867d26d8" />
...
<img width="800" alt="Sheet 4" src="https://github.com/user-attachments/assets/0bd83841-ab7e-4f62-a077-4237df20b46b" />


_Sheet 4 — MCU: ESP32-S3-WROOM-1-N4 with 10µF and 0.1µF decoupling on the 3.3V rail. I²S signals (LRCK, DOUT, BCK, SCK) are routed to the PCM5102A. UART RX/TX connect to the CP2102-GMR programming bridge. USB D+/D− are broken out for USB audio (speaker) mode. Analog inputs receive the phase difference and frequency potentiometer signals. A 10kΩ pull-up reset button and a mode selection switch with 10kΩ pull-down are included._
...
| R16 | Resistor | 680Ω 5% 0.1W | 0603 | 1 |
| R1–R4, R15 | Resistor | 5kΩ | 0603 | 5 |
| R17, R18, R19 | Resistor | 10kΩ 0.1% | 0603 | 3 |
| C1, C7, C19 | Capacitor | 1µF | 0603 | 3 |
| C2, C3, C4, C8, C11, C15, C17 | Capacitor | 0.1 µF X7R | 0603 | 7 |
| C5, C6 | Capacitor | 15nF X7R | 0603 | 2 |
| C9, C16, C18 | Capacitor | 10µF X5R | 0603 | 3 |
| C10, C14 | Capacitor | 10µF 16V | Electrolytic SMD | 2 |
| C12, C13 | Capacitor | 2.2µF X7S | 0603 | 2 |
| D1 | TVS Diode | 5V/14V | 2x2 SOND1 | 1 |
| D2 | LED | Red Clear | 2-SMD | 1 |
| RESET | Tactile Switch | MJTP1230A | THD | 1 |
...
The firmware is written in Embedded C using the ESP-IDF framework with FreeRTOS. Sine wave generation uses a phase accumulator, rather than computing sine values from absolute time, each sample increments a fixed-point phase register by a step size proportional to the desired frequency. This avoids floating-point drift and keeps both channels phase-synced over long run times.

### Architecture Highlights:
* FreeRTOS task handles I²S DMA buffer filling at audio sample rate
* State machine manages frequency/phase parameters and user input
* Phase accumulator per channel for drift-free, long-term stability
...
_Rigol DS1102E in XY mode. 210Hz on both channels with a phase offset producing a rotating disc pattern. CH1 (signal generator) on the X axis, CH2 (PCM5102A OUTR) on the Y axis._

### PCB Design
The schematic is complete in Altium Designer and PCB layout is currently underway. The design integrates the ESP32-S3, PCM5102A DAC, TLV75733PDYDR LDO, probe outputs, and USB-C connectors onto a single board.
...
- [ ] Explore OLED display for parameter readout
- [ ] Enclosure design
